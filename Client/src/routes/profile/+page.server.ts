...
export const load = (async ({ cookies: Cookies }) => {
    ...
    const response = await fetch(API_URL + "/user/get/" + username).catch(() => null);
    if (!response || !response.ok) return { message: "Server Error" };
    const data = await response.json().catch(() => null);
    if (!data) return { message: "Server Error" };
    const imageRes = await fetch(API_URL + "/user/pfp/" + username).catch(() => null);
    if (!imageRes) return { message: "Server Error" };
    const data2 = await imageRes.json().catch(() => null);
